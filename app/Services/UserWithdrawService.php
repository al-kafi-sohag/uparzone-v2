...
class UserWithdrawService
{
    ...
    public function withdrawBonus($userWithdraw)
    {
        ...
        if($user->referer_id){
            ...
            $this->userTransactionService->createTransaction($user->referer_id, $user->id, $receieveable_amount, 'Q-bit bonus from User:' . $user->id, UserTransaction::STATUS_COMPLETED, UserTransaction::TYPE_CREDIT);
        ...
        if($count > 0){
            ...
            foreach ($reffered_by as $reffered_user) {
                $this->userTransactionService->createTransaction($reffered_user->id, $user->id, $receieveable_amount, 'Q-bit bonus from User:' . $user->id, UserTransaction::STATUS_COMPLETED, UserTransaction::TYPE_CREDIT);
